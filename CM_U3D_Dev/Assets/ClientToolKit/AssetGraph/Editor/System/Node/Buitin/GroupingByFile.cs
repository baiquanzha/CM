                ...
                foreach(var ag in incoming) {
                    foreach (var g in ag.assetGroups.Keys) {
                        ...
                        foreach(var a in assets) {
                            ...
                            int len = a.path.Length - 7 - a.extension.Length;
                            string abPath = a.path.Substring(7, len);
                            string abPath1 = "";
                            int abPath1Index = abPath.IndexOf("/", StringComparison.Ordinal);
                            if (abPath1Index > -1)
                            {
                                abPath1 = abPath.Substring(abPath1Index + 1);//简而言之，就是将ResourceAB前缀给去掉
                            }
                            if (!string.IsNullOrEmpty(m_groupNameFormat[target]))
                            {
                                key = m_groupNameFormat[target]
                                    .Replace("{ABPath}", abPath)
                                    .Replace("{ABPath1}", abPath1)
